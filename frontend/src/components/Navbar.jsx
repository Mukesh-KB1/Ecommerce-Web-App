...
function Navbar() {
    ...
    const { setShowSearch, getCartCount, token, setToken, setCartItems, userId, setUserId } = UseShopContext();
    ...
    return (
        <div className='flex items-center justify-between py-5 font-medium'>
            ...
            <ul className='hidden sm:flex gap-5 text-sm text-gray-700'>
                ...
                {/* <a
                    href="https://zenvee-admin.vercel.app/"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="ml-2 px-4 py-1.5 border border-gray-300 rounded-full text-xs text-gray-700 hover:bg-gray-100 transition"
                >
                    Admin Panel
                </a> */}
            ...
            {/* Sidebar Menu Icon for small screens for Add responsiveness */}
            ...
            <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0'}`}>
                <div className='flex flex-col text-gray-600'>
                    ...
                    <NavLink onClick={() => setVisible(false)} className='py-2 pl-6 border-amber-100' to='/'>HOME</NavLink>
                    <NavLink onClick={() => setVisible(false)} className='py-2 pl-6 border-amber-100' to='/collection'>COLLECTION</NavLink>
                    <NavLink onClick={() => setVisible(false)} className='py-2 pl-6 border-amber-100' to='/about'>ABOUT</NavLink>
                    <NavLink onClick={() => setVisible(false)} className='py-2 pl-6 border-amber-100' to='/contact'>CONTACT</NavLink>
